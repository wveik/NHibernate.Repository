...
namespace RIVIERA.NHibernate.Repository.MappingClasses {
    public class HotelDictionaryWithCityMap : ClassMap<HotelDictionaryWithCity> {
        public HotelDictionaryWithCityMap() {
            Id(x => x.HD_KEY);
            Map(x => x.HD_CTKEY);
            Map(x => x.HD_CNKEY);
            Map(x => x.HD_NAME);
            ...
            Map(x => x.HD_MinAge);
            Map(x => x.REAL_HOTEL_HD_KEY);
            Map(x => x.REAL_HOTEL_HD_NAME);
            Map(x => x.HD_GROUP_WITH_WOMEN);
        }
    }
}
